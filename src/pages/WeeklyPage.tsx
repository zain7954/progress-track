...
export default function WeeklyPage() {
    ...
    const { planner, tasks, loading, handleAddTask, handleToggle, handleEdit, handleDelete } = usePlanner('weekly', weekStart);
    const { user } = useAuth();
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <CalendarDays className="w-6 h-6 text-neon-pink" />
                    <div>
                        <h1 className="text-2xl font-bold text-white">Weekly Planner</h1>
                        <p className="text-sm text-gray-400">Week starting {weekStart}</p>
                    </div>
                </div>
            ...
            {total > 0 && <SummaryCard completed={completed} total={total} rate={rate} periodLabel="This Week's" />}
        </motion.div>
    );
}
